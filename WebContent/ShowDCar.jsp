...
<%List<Car> list=(List<Car>)session2.getAttribute("Cars"); %>
<table>
<th>Car Image</th>
<th>Model:</th>
<th>Type:</th>
...
<%for(Car c:list){%>
<div class="form">
<form action="Cardetails.xyz" method="post">
<tr><td width="100"><%=c.getImage()%></td>
<td width="80"><%=c.getModel()%> <input type="hidden" name="chasisNo" value='<%=c.getChasisNumber() %>'></td>
<td width="80"><%=c.getType() %></td>
<td width="80"><%=c.getPrice() %></td>
<td width="80"><%=c.getColor() %></td>
<td  width="80"><%=c.getQuantity() %></td>
<td><input type="submit"  value="Deatils" class="button"></td>
<!-- <td><input type="submit" name="action" value="Update" class="button"></td>
<td><input type="submit" name="action" value="Delete" class="button"></td></tr> --></tr>
</form>
<%}%>
</table>
</div>
